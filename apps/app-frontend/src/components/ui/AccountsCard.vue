<template>
	<aside class="account-rail" :aria-label="formatMessage(messages.minecraftAccount)">
		<button
			v-if="offline"
			v-tooltip.left="formatMessage(messages.offlineModeDescription)"
			type="button"
			class="account-rail-status"
		...
		<div class="account-rail-list">
			<button
				v-for="account in accounts"
				:key="account.profile.id"
				v-tooltip.left="account.profile.name"
				type="button"
				class="account-rail-avatar"
				:class="{ 'is-active': selectedAccount?.profile.id === account.profile.id }"
				:aria-label="`${formatMessage(messages.selectAccount)}：${account.profile.name}`"
			...
			<p v-if="accounts.length === 0" class="account-rail-empty">{{ formatMessage(messages.notSignedIn) }}</p>
		</div>
		<div class="account-rail-footer">
			<ButtonStyled circular type="transparent">
				<OverflowMenu
					:options="addAccountOptions"
					:tooltip="formatMessage(messages.addAccount)"
				>
					<PlusIcon />
					<template #microsoft><LogInIcon /> {{ formatMessage(messages.addMicrosoftAccount) }}</template>
					<template #third-party><PlusIcon /> {{ formatMessage(messages.addThirdPartyAccount) }}</template>
					<template #offline><PlusIcon /> {{ formatMessage(messages.addOfflineAccount) }}</template>
				</OverflowMenu>
			</ButtonStyled>
		</div>
	</aside>
...
import axolotlLogo from '@/assets/lin-client.png'
...
const addAccountOptions = computed(() => [
	{ id: 'microsoft', action: () => void login(), disabled: offline.value || loginDisabled.value },
	{
		id: 'third-party',
		action: () => showYggdrasilAccountModal(),
		disabled: offline.value || loginDisabled.value,
	},
	{ id: 'offline', action: () => showOfflineAccountModal(), disabled: loginDisabled.value },
])
...
const messages = defineMessages({
	addAccount: { id: 'minecraft-account.add-account', defaultMessage: 'Add account' },
	offlineMode: {
...
<style scoped>
.account-rail { display:flex; width:4.5rem; height:100%; min-height:0; box-sizing:border-box; flex-direction:column; align-items:center; padding:.75rem .45rem; border:1px solid rgb(15 23 42 / 8%); border-radius:2rem; background:linear-gradient(180deg,#fff,#f7f8fa); box-shadow:0 .75rem 2rem rgb(15 23 42 / 10%); }
.account-rail-list { display:flex; width:100%; min-height:0; flex:1; flex-direction:column; align-items:center; gap:.65rem; overflow-y:auto; scrollbar-width:none; }
.account-rail-list::-webkit-scrollbar { display:none; }
.account-rail-avatar,.account-rail-status { position:relative; display:grid; width:3rem; height:3rem; flex:0 0 auto; place-items:center; padding:.2rem; border:0; border-radius:1rem; background:transparent; color:#273142; cursor:pointer; transition:transform 140ms ease,background-color 140ms ease; }
.account-rail-avatar:hover,.account-rail-status:hover { transform:translateY(-1px); background:#eef0f4; }
.account-rail-avatar.is-active { background:#e9ebef; box-shadow:none; }
.account-rail-avatar.is-active::before { position:absolute; left:-.34rem; width:.2rem; height:1rem; content:''; border-radius:999px; background:#252b36; }
.account-rail-avatar :deep(*) { border:0 !important; outline:0 !important; box-shadow:none !important; }
.account-rail-avatar :deep(img) { border-radius:.72rem !important; }
.account-rail-status { margin-bottom:.65rem; color:var(--color-orange); }
.account-rail-empty { margin:auto 0; color:#737b88; font-size:.65rem; line-height:1.35; text-align:center; }
.account-rail-footer { display:flex; width:100%; justify-content:center; margin-top:.65rem; padding-top:.75rem; border-top:1px solid #e2e4e8; }
.account-rail-footer :deep(button) { width:3.2rem; height:3.2rem; border:2px solid #d8dbe1 !important; border-radius:1rem !important; background:#eef0f4 !important; color:#202631 !important; box-shadow:0 .25rem .7rem rgb(15 23 42 / 12%) !important; }
.account-rail-footer :deep(button:hover) { background:#e2e5ea !important; transform:translateY(-1px); }
.account-rail-footer :deep(svg) { width:1.35rem; height:1.35rem; stroke-width:2.5; }
</style>
