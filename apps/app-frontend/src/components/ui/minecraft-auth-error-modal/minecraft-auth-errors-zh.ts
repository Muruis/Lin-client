const translations: Record<string, string> = {
	'Your saved Microsoft sign-in token has expired or was revoked, so Lin Client cannot refresh your Minecraft session.':
		'已保存的 Microsoft 登录令牌已过期或被撤销，因此 Lin Client 无法刷新你的 Minecraft 会话。',
	'Sign out of the affected Minecraft account in Lin Client':
		'在 Lin Client 中退出受影响的 Minecraft 账号',
	...
	'Use the sync option in your system settings to synchronize the clock':
		'使用系统设置中的同步选项校准时钟',
	'Restart Lin Client': '重启 Lin Client',
	...
	'Restart Lin Client after waiting': '等待后重启 Lin Client',
	...
	"Minecraft's authentication service is returning a server error, so Lin Client cannot finish signing you in right now.":
		'Minecraft 身份验证服务返回了服务器错误，Lin Client 目前无法完成登录。',
	...
	'Make sure you are using the Microsoft account that owns Minecraft. Visit <a href="https://github.com/Mystic-Stars/Lin Client/issues">Lin Client support</a> for help':
		'确认当前使用的是拥有 Minecraft 的 Microsoft 账号。如需帮助，请访问 <a href="https://github.com/Mystic-Stars/Lin Client/issues">Lin Client 支持</a>',
	'Try signing in to Lin Client again': '再次尝试登录 Lin Client',
	'Lin Client could not connect to a Microsoft, Xbox, or Minecraft service needed for sign-in. This is usually caused by a local network, DNS, proxy, firewall, hosts file, VPN, or antivirus issue.':
		'Lin Client 无法连接登录所需的 Microsoft、Xbox 或 Minecraft 服务。通常是本地网络、DNS、代理、防火墙、hosts 文件、VPN 或杀毒软件导致的。',
	'Restart Lin Client and try signing in again': '重启 Lin Client，然后再次尝试登录',
	'Check that your internet connection is working': '检查网络连接是否正常',
	'Allow Lin Client through your firewall, antivirus, proxy, VPN, and hosts file rules':
		'在防火墙、杀毒软件、代理、VPN 和 hosts 文件规则中允许 Lin Client 通行',
	...
	'Xbox services rejected the request to authorize this account for Minecraft services, but did not return a specific account restriction that Lin Client recognizes.':
		'Xbox 服务拒绝授权此账号访问 Minecraft 服务，但未返回 Lin Client 能识别的具体账号限制。',
