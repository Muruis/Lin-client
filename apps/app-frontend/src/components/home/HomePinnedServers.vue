...
const messages = defineMessages({
	...
	manage: { id: 'app.home.servers.manage', defaultMessage: 'Add or edit servers' },
})
...
<template>
	<section class="home-pinned-servers flex min-w-0 min-h-0 h-full flex-col gap-3" :data-size="dashboardSize">
		<div class="home-widget-heading flex min-w-0 h-8 flex-none items-center gap-2">
			...
			<ButtonStyled v-if="instances[0]" type="transparent" size="small" class="ml-auto">
				<router-link :to="`/instance/${instances[0].id}/worlds`">
					<PencilIcon aria-hidden="true" />
					<span>{{ formatMessage(messages.manage) }}</span>
				</router-link>
			</ButtonStyled>
		</div>
