					...
					>
						...
						<div class="home-widget-content min-w-0 min-h-0 flex-1 overflow-hidden p-4">
							...
							<HomeCalendar
								...
								:dashboard-size="effectiveSize(widget)"
							/>
							<HomePlayInsights v-else-if="widget.kind === 'play-insights'" />
							<HomePinnedInstances
...
.home-dashboard {
	...
	border-radius: 2rem;
	background: color-mix(in srgb, var(--color-bg) 94%, white);
	box-shadow: 0 1.25rem 3rem rgb(15 23 42 / 8%);
}
...
.home-widget {
	...
	box-shadow: var(--shadow-card), 0 0.7rem 1.5rem rgb(15 23 42 / 7%);
	transition:
