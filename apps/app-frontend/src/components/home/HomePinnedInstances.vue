...
const cardLayout = computed(() => {
	...
	if (props.dashboardSize === '2x2' || props.dashboardSize === '3x2') return 'tile' as const
	return 'row' as const
})
...
<template>
	<section class="home-pinned-instances flex min-w-0 min-h-0 h-full flex-col gap-3" :data-size="dashboardSize">
		...
		<div v-if="pinnedInstances.length > 0" class="home-instance-list grid min-w-0 min-h-0 flex-1 grid-auto-rows-max gap-3 overflow-x-hidden overflow-y-auto pr-1">
			<HomeInstanceCard
...
.home-pinned-instances[data-size='3x2'] .home-instance-list {
