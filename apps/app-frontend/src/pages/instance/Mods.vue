<template>
	<ReadyTransition :pending="loading">
		...
		<div class="mods-action-bar mb-4 flex flex-wrap items-center justify-between gap-3">
			<div class="min-w-0">
				<h2 class="m-0 text-base font-bold text-contrast">{{ formatMessage(messages.manageMods) }}</h2>
				<p class="m-0 mt-1 text-sm text-secondary">{{ formatMessage(messages.manageModsHint) }}</p>
			</div>
			<div class="flex flex-wrap items-center gap-2">
				<ButtonStyled color="brand">
					<button type="button" @click="openModsFolder">
						<FolderOpenIcon aria-hidden="true" />
						{{ formatMessage(messages.openModsFolder) }}
					</button>
				</ButtonStyled>
			</div>
		</div>
...
import { openPath as openLocalPath } from '@/helpers/utils'
...
const messages = defineMessages({
	manageMods: { id: 'app.instance.mods.manage', defaultMessage: 'Manage Mods' },
	manageModsHint: {
		id: 'app.instance.mods.manage-hint',
		defaultMessage: 'Open the folder and manage .jar Mods directly.',
	},
	openModsFolder: { id: 'app.instance.mods.open-folder', defaultMessage: 'Open Mods folder' },
	shareTitle: {
...
async function openModsFolder() {
	if (!props.instance) return
	try {
		const instancePath = await get_full_path(props.instance.id)
		await openLocalPath(await join(instancePath, 'mods'))
	} catch (error) {
		handleError(error as Error)
	}
}
