...
const messages = defineMessages({
	...
	runningAsAdmin: {
		...
		defaultMessage:
			'Lin Client is running as administrator. Drag-and-drop file import is disabled in this mode; please restart the launcher without administrator privileges.',
	},
	restarting: {
		id: 'app.restarting',
	...
	closeLauncherTitle: {
		...
		defaultMessage: 'Choose how to close Lin Client',
	},
	closeLauncherDirect: {
...
async function setupApp() {
	...
	// Lin Client currently ships with a single Simplified Chinese interface.
	i18n.global.locale.value = 'zh-CN'
	if (locale !== 'zh-CN') {
		initialSettings.locale = 'zh-CN'
	...
	// Lin Client uses a standalone startup animation and does not show the
	// upstream interactive launcher or instance tutorials.
	if (!initialSettings.onboarded || !initialSettings.onboarding_instance_tour_completed) {
		initialSettings.onboarded = true
		initialSettings.onboarding_version = 1
		initialSettings.onboarding_instance_tour_completed = true
		await setSettings(initialSettings)
		onboardingSettings.value = initialSettings
	}
...
const updatePopupMessages = defineMessages({
	...
	meteredBody: {
		...
		defaultMessage: `Lin Client v{version} is available now! Since you're on a metered network, we didn't automatically download it.`,
	},
	downloadedBody: {
		id: 'app.update-popup.body.download-complete',
		defaultMessage: `Lin Client v{version} has finished downloading. Reload to update now, or automatically when you close Lin Client.`,
	},
	reload: {
		...
		>
			...
			>
				...
				<div class="sidebar-default-content hidden" :class="{ 'sidebar-enabled': sidebarVisible }">
					<div class="account-rail-host h-full p-1.5">
						<suspense>
...
.app-contents {
	...
	&.sidebar-enabled:has(.sidebar-teleport-content:empty) {
		--right-bar-width: 5.25rem;
	}
...
.app-contents:has(.sidebar-teleport-content:empty) .app-sidebar {
	width: 5.25rem;
	background: transparent;
	border-left: 0;
}

.app-contents:has(.sidebar-teleport-content:empty) .sidebar-toggle-handle,
.app-contents:has(.sidebar-teleport-content:empty) .app-sidebar::before,
.app-contents:has(.sidebar-teleport-content:empty) .app-sidebar-scrollable :deep(.os-scrollbar) {
