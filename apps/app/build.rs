...
fn build_blockbench_skin_editor() {
    ...
    if !blockbench_dir.join("package.json").is_file() {
        println!(
            "cargo:warning=Blockbench skin editor source is unavailable; building without the embedded skin editor"
        );
        std::fs::create_dir_all("resources/blockbench-skin")
            .expect("Failed to create the empty Blockbench resource directory");
        return;
    }
